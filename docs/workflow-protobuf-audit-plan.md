...
- [x] `AUDITED` Treat the workflow proto package as an internal durable-message package,
  not a public RPC package. Do not add workflow RPC services unless an internal
  transport boundary is introduced later.
- [x] `AUDITED` Let general Protocol Buffers best practices constrain the internal
...
- [x] `AUDITED` The hard rewrite removes current JSON compatibility paths, but it does not
  waive future schema-evolution rules once the new protobuf schemas land.

## Target Shape

- [x] `AUDITED` Treat workflow commands, events, effects, and durable replay payloads as
  internal protobuf contracts, not hand-written JSON/Zod contracts.
- [x] `AUDITED` Keep reducers pure: reducers receive already-decoded command/event values
  and emit already-typed events/effects.
- [x] `AUDITED` Keep effects isolated: effect runners encode their result commands through
  the same internal protobuf boundary used for replay.
- [x] `AUDITED` Keep public Connect RPC messages separate from durable workflow messages.
  Do not store public `ExecuteQueryResponse` or `SourceApiExecutionResult`
  messages directly as audit-log truth.
- [x] `AUDITED` Treat package names as contract lifecycle boundaries, not generated-code
...
- [ ] After this storage migration lands, continue with
  ...
  package rewrites.
- [x] `AUDITED` Store protobuf binary payloads in audit tables, while retaining scalar
  columns such as `family`, `command_type`, `event_type`, and `effect_type` for
  querying, idempotency, and diagnostics.
- [x] `AUDITED` Keep idempotency, effect keys, diagnostic lookup, and cache/fingerprint
  inputs based on explicit scalar/domain values, not on raw serialized protobuf
  bytes.
- [x] `AUDITED` Remove the duplicate Zod payload schemas from query/source-api workflow
  codecs after protobuf decode/validate covers the durable boundary.

## References

- [x] `AUDITED` Use `.tmp/connect-protobuf-es-repo/MANUAL.md` for protobuf-es message
  creation, `toBinary`, `fromBinary`, `toJson`, and `fromJson` conventions.
- [x] `AUDITED` Use `.tmp/connect-protobuf-es-repo/packages/protoc-gen-es/README.md` for
  `json_types=true`, `valid_types=protovalidate_required`, and generation
  option tradeoffs.
- [x] `AUDITED` Use `.tmp/connect-connectrpc.com-repo/docs/node/interceptors.md` as the
  reference for keeping public RPC validation at the Connect interceptor layer.
- [x] `AUDITED` Use `.tmp/connect-connectrpc.com-repo/docs/go/serialization-and-compression.md`
  for the schema-first JSON/binary separation in Connect.
- [x] `AUDITED` Use `.tmp/connect-es-repo/README.md` for the local Connect-ES example of
...
- [x] `AUDITED` Add an internal proto package under `proto/onequery/workflow/v1/`.
- [x] `AUDITED` Keep `onequery/workflow/v1/**` independent from `onequery/cli/v1/**`.
...
- [x] `AUDITED` Create `proto/onequery/workflow/v1/query_action.proto`.
- [x] `AUDITED` Create `proto/onequery/workflow/v1/source_api_action.proto`.
- [x] `AUDITED` Create a shared file if useful, for example
...
- [x] `AUDITED` Use `edition = "2023";` to match the existing `proto/onequery/cli/v1/*`
  files.
- [x] `AUDITED` Do not add edition feature overrides unless a specific workflow contract
  requires one and the reason is documented near the file option.
- [x] `AUDITED` Import `buf/validate/validate.proto` and add Protovalidate constraints to
  all production fields.
- [x] `AUDITED` Model command payloads as one `oneof` per workflow family:
  `QueryActionCommandPayload` and `SourceApiActionCommandPayload`.
- [x] `AUDITED` Model events as one `oneof` per workflow family:
  `QueryActionEventPayload` and `SourceApiActionEventPayload`.
- [x] `AUDITED` Model effects as one `oneof` per workflow family:
  `QueryActionEffectPayload` and `SourceApiActionEffectPayload`.
- [x] `AUDITED` Mark command/event/effect payload oneofs with
  `(buf.validate.oneof).required = true` so an empty wrapper is rejected before
  domain conversion.
- [x] `AUDITED` Keep the scalar type column and protobuf oneof case redundant by design:
...
- [x] `AUDITED` Model query execution replay data as an internal message, for example
  `QueryActionRecordQueryExecutionCommand`, containing the full rows/columns
  response needed by replay.
- [x] `AUDITED` Model source-api page replay data as an internal message, including binary
  response bodies as `bytes`, not base64 strings.
- [x] `AUDITED` Use `bytes` for arbitrary provider response/request bodies and other
  non-UTF-8 payloads. Use `google.protobuf.Value` only for JSON-shaped provider
  state that is already semantically JSON.
- [x] `AUDITED` Use `google.protobuf.Timestamp` and `google.protobuf.Duration` for time
  points and spans if they enter durable protobuf payloads.
- [x] `AUDITED` Avoid `google.protobuf.Any`; use explicit oneofs or typed internal
  messages for all known workflow payload variants.
- [x] `AUDITED` Keep nullable domain concepts explicit with optional message fields or
  oneofs rather than sentinel strings.
- [x] `AUDITED` Avoid reusing public CLI API response messages for storage unless the
  field is truly part of the durable workflow truth.
- [x] `AUDITED` Replace `WorkflowCliProblemKey` with workflow-owned failure semantics, or
...
- [x] `AUDITED` Add comments to fields whose retention exists for deterministic replay
  rather than UI presentation.
- [x] `AUDITED` Add enum `UNSPECIFIED = 0` values and reject them at decode/validation
  boundaries before reducers see them.
- [x] `AUDITED` Use dense enum numbering for new enums, reserve removed enum numbers and
  names, and never recycle enum names for a different meaning.
- [x] `AUDITED` Never reuse field numbers. If a field is removed after the new schema is
...
- [x] `AUDITED` Update `proto/buf.gen.yaml` to keep generating TypeScript into
  `packages/cli-server/src/connect/gen`.
  Existing template output already covered the new internal workflow package.
- [x] `AUDITED` Consider enabling `json_types=true` for generated internal protobuf types
...
- [x] `AUDITED` Enable or evaluate `valid_types=protovalidate_required` so required
...
- [x] `AUDITED` Verify `proto/buf.gen.yaml` uses protoc-gen-es option spelling accepted by
  the current generator; prefer documented options when generated output is
  unchanged.
- [x] `AUDITED` Keep `@bufbuild/protobuf`, `@bufbuild/protoc-gen-es`, and generated code
  versions aligned through the workspace catalog.
- [x] `AUDITED` Run `bun run proto:generate` after adding proto files.
- [x] `AUDITED` Confirm generated files remain under the configured `proto/buf.gen.yaml`
...
- [x] `AUDITED` Run `buf format -w proto` and `buf lint proto`.

## Storage Model

- [x] `AUDITED` Replace `workflow_commands.command_payload_json` with a protobuf binary
  payload column, for example `command_payload_bytes`.
- [x] `AUDITED` Replace `query_action_events.payload_json` with `payload_bytes`.
- [x] `AUDITED` Replace `source_api_action_events.payload_json` with `payload_bytes`.
- [x] `AUDITED` Replace `workflow_effect_dispatches.payload_json` with `payload_bytes`.
- [x] `AUDITED` Keep `workflow_commands.command_type`, event tables' `event_type`, and
  `workflow_effect_dispatches.effect_type` as indexed scalar columns.
- [x] `AUDITED` Keep JSON projection columns only where they are first-class read models,
  not as the canonical workflow payload store.
- [x] `AUDITED` Do not store ProtoJSON/text-format payloads as canonical workflow truth.
  If a UI/debug feed needs JSON, derive it from decoded protobuf/domain values.
- [x] `AUDITED` Do not compare or hash raw serialized protobuf bytes for idempotency,
  effect keys, or replay legality. Protobuf serialization is not a stable
  cross-build identity format.
- [x] `AUDITED` Generate a destructive Drizzle migration for the column rewrite. Do not
  edit `packages/db/src/migrations/**` manually.
- [x] `AUDITED` Update PGlite test setup and fixtures so fresh schemas are enough; no
  compatibility migration fixtures are required.
- [x] `AUDITED` Remove `WorkflowJson` usage from canonical workflow payload columns after
  the binary columns land.

## Codec Boundary

- [x] `AUDITED` Add a small codec module per workflow family near the audit family code,
  for example `query-action-family/protobuf-codec.ts`.
- [x] `AUDITED` Encode command payloads with `create(Schema, init)` and
  `toBinary(Schema, message)`.
- [x] `AUDITED` Decode command payloads with `fromBinary(Schema, bytes)`.
- [x] `AUDITED` Validate decoded internal messages with `@bufbuild/protovalidate` before
  converting to domain unions.
- [x] `AUDITED` Convert generated protobuf oneofs to the existing domain ADTs at one
  boundary only.
- [x] `AUDITED` Convert domain ADTs to generated protobuf oneofs at one boundary only.
- [x] `AUDITED` Return typed corruption errors on decode/validation failure, preserving
  the current distinction between corrupt audit state and normal domain failure.
- [x] `AUDITED` Keep protobuf enum conversion helpers narrow and exhaustive.
- [x] `AUDITED` Reject unknown, unspecified, or unsupported enum values before converting
  generated messages to domain commands/events/effects.
- [x] `AUDITED` Reject mismatches between stored scalar type columns and decoded protobuf
  oneof cases as audit corruption.
  Fixed: command storage now writes exact command-payload oneof-case types, not
  grouped reducer ADT types such as `record_query_execution`.
- [x] `AUDITED` Avoid byte-equality assertions or canonical-byte assumptions in codec
  logic; decode bytes, validate, then compare semantic/domain values.
- [x] `AUDITED` Add compile-time exhaustiveness checks for every oneof-to-domain switch.

## Query Workflow Cleanup

- [x] `AUDITED` Replace `QueryActionCommandPayload` hand-written union ownership with the
  ...
  domain ADT.
- [x] `AUDITED` Remove `StoredQueryValidationResultPayloadSchema` from
  `packages/cli-server/src/connect/service/query/workflow-codec.ts`.
- [x] `AUDITED` Remove `StoredQueryExecutionResultPayloadSchema` from
  `packages/cli-server/src/connect/service/query/workflow-codec.ts`.
- [x] `AUDITED` Keep `toStoredQueryExecutionResult` or rename it to a pure projector that
  accepts an already-decoded `record_query_execution` command payload.
- [x] `AUDITED` Move command-payload validation into stored command loading, not replay
  projection.
- [x] `AUDITED` Make `loadStoredAcceptedQueryActionResultCommand` return decoded command
  payloads.
- [x] `AUDITED` Keep event replay functions based on committed events where events contain
  the full truth, for example source lookup and credentials load.
- [x] `AUDITED` Keep query execution replay based on the stored result command because
  committed query execution events intentionally store only summary data.

## Source API Workflow Cleanup

- [x] `AUDITED` Replace source-api stored execution result JSON/base64 encoding with the
  internal protobuf `bytes` body representation.
- [x] `AUDITED` Remove duplicate Zod payload schemas from
...
- [x] `AUDITED` Keep source-api descriptor and request-preparation replay projections pure
  and typed.
- [x] `AUDITED` Make page-fetch replay decode a typed protobuf result command before
  projecting to runtime `PageFetchResult`.
- [x] `AUDITED` Confirm continuation state remains a protobuf-supported JSON value
  boundary, for example `google.protobuf.Value`, and is not cast with `as never`.

## Reducers And State Machines

- [x] `AUDITED` Keep `decideQueryAction` and `decideSourceApiAction` accepting domain
  commands, not raw protobuf messages.
- [x] `AUDITED` Keep reducers accepting domain events, not raw protobuf messages.
- [x] `AUDITED` Ensure every reducer transition still defines truth and no effect runner
  updates state directly.
- [x] `AUDITED` Remove any reducer-level shape guards made redundant by protobuf boundary
  decoding.
- [x] `AUDITED` Keep normal failure/retry lifecycle states modeled as command/event
...
- [x] `AUDITED` Update audit feed projection code to consume decoded protobuf payloads.
- [x] `AUDITED` Keep read models separate from canonical workflow payloads.
- [x] `AUDITED` If audit feed needs JSON for UI/debugging, derive it from decoded
  protobuf/domain values instead of reading JSON payload columns.
- [x] `AUDITED` Update corruption diagnostics to include family, command id, action id,
  and payload type, without logging raw row/result bodies unnecessarily.
  Fixed: audit-feed projection now carries command ids through event records,
  validates decoded protobuf payloads, and wraps corrupt payload failures with
  family/action/command/event/payload diagnostics instead of raw bytes.
