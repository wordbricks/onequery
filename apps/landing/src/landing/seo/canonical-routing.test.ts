...
describe("canonical routing", () => {
  it("keeps canonical extensionless paths unchanged", () => {
  ...
  it("removes trailing slashes from non-root paths", () => {
    expect(getCanonicalPathRedirect("/blog/")).toEqual({ pathname: "/blog" });
    expect(getCanonicalPathRedirect("/blog/category/product/")).toEqual({
