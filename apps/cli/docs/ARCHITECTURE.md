...
- Config targeting: the public `config set server <url>` and `config get <key>` commands persist and inspect CLI defaults through the same `config/` ownership boundary that stores active org selection and other local defaults.
