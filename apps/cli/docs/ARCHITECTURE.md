...
- Serve lifecycle command surface: `commands/serve.rs` owns the public local `oneq serve*` surface, including the foreground `oneq serve` launcher that resolves packaged Bun runtime assets and the shared local runtime-state inspection used by `serve status`, `serve logs`, and the remaining lifecycle subcommands.
...
- Config targeting: the public `config set server <url>` command persists the CLI's default server target through the same `config/` ownership boundary that stores active org selection and other local defaults.
