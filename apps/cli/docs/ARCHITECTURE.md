...
- Gateway lifecycle command surface: `commands/gateway/mod.rs` owns the public local `onequery gateway*` surface, including the foreground `onequery gateway` launcher that resolves packaged Bun runtime assets and the shared local runtime-state inspection used by `gateway status`, `gateway logs`, and the remaining lifecycle subcommands.
