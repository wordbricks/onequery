...
- Follow `apps/cli/docs/skills/SKILL.md` and branch immediately into the help-first path when the task is just command discovery.
- Prefer `onequery --help`, `onequery <command> --help`, and `onequery schema skills --output json` before guessing command shapes.
- Resolve org and source ambiguity before proposing SQL.

## Workflow

1. Use CLI help and `onequery schema skills` when the task is command or workflow discovery.
