...
2. For protected workflows, confirm auth with `oneq auth whoami`.
3. Resolve org context with `oneq org current` and `oneq org list` when needed.
4. Resolve sources with `oneq source list` and confirm query support with `oneq source show <source_key>`.
