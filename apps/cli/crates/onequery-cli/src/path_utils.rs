...
impl AbsolutePathBuf {
    ...
    pub(crate) fn resolve_path_against_base<P: AsRef<Path>, B: AsRef<Path>>(
        path: P,
        base_path: B,
    ) -> io::Result<Self> {
    ...
    pub(crate) fn from_absolute_path<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        let normalized_path = Self::normalize_cli_input_path(path.as_ref());
    ...
    #[allow(dead_code)]
    pub(crate) fn current_dir() -> io::Result<Self> {
        Self::from_absolute_path(std::env::current_dir()?)
    }

    #[allow(dead_code)]
    pub(crate) fn join<P: AsRef<Path>>(&self, path: P) -> io::Result<Self> {
...
pub(crate) fn resolve_user_path_for_cli(
    path: &Path,
    command_line: &str,
    stage: ErrorStage,
...
pub(crate) fn resolve_env_directory_for_cli(
    env_name: &str,
    path: &Path,
    command_line: &str,
    stage: ErrorStage,
...
#[derive(Debug, Clone, Eq, PartialEq)]
pub(crate) struct SymlinkWritePaths {
    pub(crate) read_path: Option<PathBuf>,
...
pub(crate) fn resolve_symlink_write_paths(path: &Path) -> io::Result<SymlinkWritePaths> {
...
pub(crate) fn write_atomically(write_path: &Path, contents: &str) -> Result<(), AtomicWriteError> {
