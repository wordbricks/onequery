...
pub use onequery_utils_path::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\onequery.zip").as_deref(),
            Some("/mnt/c/Temp/onequery.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/onequery.tgz").as_deref(),
            Some("/mnt/d/Work/onequery.tgz")
        );
        assert!(win_path_to_wsl("/home/user/onequery").is_none());
    }
