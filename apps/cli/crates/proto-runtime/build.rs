...
fn discover_proto_files(repo_root: &Path) -> Vec<PathBuf> {
    let runtime_proto_dir = repo_root.join(PROTO_ROOT).join(RUNTIME_PROTO_DIR);
    let entries = std::fs::read_dir(&runtime_proto_dir).unwrap_or_else(|error| {
        panic!(
            "expected runtime proto directory {}: {error}",
            runtime_proto_dir.display()
        )
    });
    let mut proto_files = entries
        .map(|entry| {
            entry.unwrap_or_else(|error| {
                panic!(
                    "expected to read runtime proto directory entry under {}: {error}",
                    runtime_proto_dir.display()
                )
            })
        })
    ...
    if proto_files.is_empty() {
        panic!(
            "expected at least one runtime proto file under {}",
            runtime_proto_dir.display()
        );
    }
...
fn emit_rerun_triggers(repo_root: &Path, proto_files: &[PathBuf]) {
    ...
    println!(
        "cargo:rerun-if-changed={}",
        proto_root.join(RUNTIME_PROTO_DIR).display()
    );
...
fn build_descriptor_set(repo_root: &Path, descriptor_path: &Path, runtime_proto_entrypoint: &Path) {
    let proto_root = repo_root.join(PROTO_ROOT);
    ...
    for candidate in candidates {
        ...
        // Buf config lives under proto/, so descriptor builds must run from
        // that workspace root while the generated descriptor lands in OUT_DIR.
        buf_command
    ...
    panic!(
        "expected to find `buf` for descriptor generation. Tried: {}. \
         ...
         or run `nix shell nixpkgs#buf nixpkgs#protobuf --command cargo test -p onequery-proto-runtime`",
