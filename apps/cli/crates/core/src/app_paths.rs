...
pub fn config_path(command_line: &str) -> Result<PathBuf, CliError> {
    Ok(config_dir(command_line)?.join(CONFIG_FILENAME))
}
...
pub fn data_dir(command_line: &str) -> Result<PathBuf, CliError> {
    Ok(onequery_home(command_line, "data")?.join("data"))
}

/// Resolves the application state directory.
pub fn state_dir(command_line: &str) -> Result<PathBuf, CliError> {
    Ok(onequery_home(command_line, "state")?.join("state"))
}
