...
///
/// This is expected to be used in test code because OneQuery CLI is a
/// standalone binary with no packaged test resources.
#[macro_export]
macro_rules! find_resource {
    ($resource:expr) => {{
        let resource = std::path::Path::new(&$resource);
        $crate::resolve_cargo_runfile(resource)
    }};
}
