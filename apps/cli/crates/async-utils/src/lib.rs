...
/// Adds cancellation-token handling to futures.
///
/// Implementations should return the wrapped future's output when it completes first, or
/// `CancelErr::Cancelled` when the cancellation token fires first.
#[async_trait]
