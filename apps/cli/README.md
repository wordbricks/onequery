...
- Packaged `onequery serve` uses a bundled Rolldown-built server runtime and launches it with Node.js. The hosted `install.sh` bootstrap now installs managed official Node.js 24.x on macOS/Linux when `node` 24+ is not already available. Direct npm/bun installs still require Node.js 22+ on `PATH` or `ONEQUERY_SERVER_JS_RUNTIME`.
